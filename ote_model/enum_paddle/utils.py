...
def data_preprocess(file_name, ratio=3, opinion_level='segment', language=\
    'english'):
    ...
    for doc in docs:
        ...
        for doc_opinion in doc_opinions:
            opinion_sents = sents[doc_opinion['start_sent_idx']:doc_opinion
                ['end_sent_idx'] + 1]
            ...
            try:
                ...
                argument = ''
            inst = [opinion_text, spans[event], event, argument, event_id,
    ...
    input_insts = []
    for inst in insts:
        input_insts.append([inst[0], inst[3], 1, inst[3], inst[4], inst[5],
            inst[6], inst[7]])
        choice_spans = random.sample(inst[1], min(ratio, len(inst[1])))
        for choice_span in choice_spans:
            if choice_span != inst[3]:
                input_insts.append([inst[0], choice_span, 0, inst[3], inst[
                    4], inst[5], inst[6], inst[7]])
    data = pd.DataFrame(input_insts)
    data.columns = ['sent', 'span', 'is_aspect', 'gold_aspect', 'event_id',
        'doc_id', 'start_sent_idx', 'end_sent_idx']
    return data
...
def data_batch(data_df, batch_size, shuffle=True):
    ...
    inputs = list(zip(text_a, text_b, labels, gold_aspect, event_id, doc_id,
    ...
    for i in tqdm(range(0, len(inputs), batch_size)):
        ...
        (batch_text_a, batch_text_b, batch_labels, batch_gold_aspect,
            batch_event_id, batch_doc_id, batch_start_sent_idx,
            batch_end_sent_idx) = zip(*batch)
        tokenized_input = tokenizer(batch_text_a, batch_text_b, max_length=\
            512, padding='max_length', truncation='longest_first')
        input_ids, token_type_ids, attention_mask = (tokenized_input.
            input_ids, tokenized_input.token_type_ids, tokenized_input.
            attention_mask)
        batch_input = (torch2paddle.create_tensor(input_ids), torch2paddle.
            create_tensor(token_type_ids), torch2paddle.create_tensor(
            attention_mask), torch2paddle.create_tensor(batch_labels),
            batch_text_a, batch_text_b, batch_gold_aspect, batch_event_id,
            batch_doc_id, batch_start_sent_idx, batch_end_sent_idx)
