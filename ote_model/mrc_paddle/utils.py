...
def read_squad_examples(input_file, language='english'):
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    version_2_with_negative, null_score_diff_threshold): #TODO 修改预测方式
    ...
    _PrelimPrediction = collections.namedtuple('PrelimPrediction', [
        ...
        'end_logit', 'span_logit'])
    all_predictions = collections.OrderedDict()
    all_nbest_json = collections.OrderedDict()
    scores_diff_json = collections.OrderedDict()
    
    """处理all_examples的循环
    example_index_to_features: 存储特征的列表
    unique_id_to_result: 存储结果的列表

    Returns:
        all_predictions, all_nbest_json, scores_diff_json
    """
    for example_index, example in enumerate(all_examples):
        features = example_index_to_features[example_index] # 提取特征
        prelim_predictions = [] # 存储预测结果
        score_null = 1000000
        min_null_feature_index = 0
        null_start_logit = 0
        null_end_logit = 0
        """循环处理features(所有可能的span)"""
        for feature_index, feature in enumerate(features):
            result = unique_id_to_result[feature.unique_id]
            # start_indexes = _get_best_indexes(result.start_logits, n_best_size) # 选取前20个start_logits最高的span的下标
            # end_indexes = _get_best_indexes(result.end_logits, n_best_size)
            # print("[INFO] result.start_logits shape is {}".format(np.array(result.start_logits).shape))
            # print("[INFO] result.end_logits shape is {}".format(np.array(result.end_logits).shape))
            # print("[INFO] result.span_logits shape is {}".format(np.array(result.span_logits).shape))
            span_indexes = _get_best_start_end_indexes(result.span_logits, n_best_size) # 找到分数最高的400个span，返回他们的start和end
            if version_2_with_negative: # 默认不执行，不用管
            ...
            # 筛掉不符合条件的span，组成候选span--prelim_predictions
            max_len = int(pow(len(result.span_logits),0.5)) # max_len = 512
            for span_index in span_indexes:
                start_index = span_index//max_len
                end_index = span_index%max_len
                ...
                prelim_predictions.append(_PrelimPrediction(
                    ...
                    end_logits[end_index],span_logit=result.span_logits[span_index]))
        ...
        #TODO 根据总分数为候选span排序，修改这里的排序方式应该就可以了
        prelim_predictions = sorted(prelim_predictions, key=lambda x: x.span_logit, reverse=True)
        _NbestPrediction = collections.namedtuple('NbestPrediction', [
            'text', 'start_logit', 'end_logit', 'span_logit'])
        seen_predictions = {}
        nbest = []
        
        # 对前n_best_size个答案的文本进一步处理，形成最好的几个答案，保存在nbest列表中
        for pred in prelim_predictions:
            ...
            nbest.append(_NbestPrediction(text=final_text, start_logit=pred
                .start_logit, end_logit=pred.end_logit, span_logit=pred.span_logit))
        ...
        if not nbest:
            nbest.append(_NbestPrediction(text='empty', start_logit=0.0,
                end_logit=0.0, span_logit=0.0))
        assert len(nbest) >= 1
        
        total_scores = []
        best_non_null_entry = None
        #TODO 计算总分，形成total_scores列表，这里改一下计算总分的方式
        for entry in nbest:
            total_scores.append(entry.span_logit)
            if not best_non_null_entry:
                if entry.text:
                    best_non_null_entry = entry
        probs = _compute_softmax(total_scores)
        
        # 形成nbest_json为写入json做准备
        nbest_json = []
        for i, entry in enumerate(nbest):
            ...
            output['span_logit'] = entry.span_logit
            nbest_json.append(output)
        assert len(nbest_json) >= 1
        
        if not version_2_with_negative:
            # 形成预测all_predictions，取最佳答案的文本
...
def write_predictions_extended(all_examples, all_features, all_results,
    ...
    example_index_to_features = collections.defaultdict(list)
    for feature in all_features:
        example_index_to_features[feature.example_index].append(feature) # 建立example_index到result的索引
    unique_id_to_result = {}
    for result in all_results:
        unique_id_to_result[result.unique_id] = result # 建立unique_id到result的索引
    all_predictions = collections.OrderedDict()
    all_nbest_json = collections.OrderedDict()
    scores_diff_json = collections.OrderedDict()
    for example_index, example in enumerate(all_examples):
...
def _get_best_indexes(logits, n_best_size): # 输入形状为 [512,128] 原生paddle实现的形状是[512]
...
def _get_best_start_end_indexes(logits, n_best_size): # 输入形状为[512*512]
    n_best_size = pow(n_best_size,2)
    ...
    for i in range(len(index_and_score)):
        if i >= n_best_size:
            break
        span_indexes.append(index_and_score[i][0])
        
    # start_indexes=[]
    # end_indexes=[]
    # for x in best_indexes:
    #     start_indexes.append(x//max_len)
    #     end_indexes.append(x%max_len)
    return span_indexes
