...
"""
注意,
paircls是按句子级预测,所以预测前要转化为篇章
seq是按篇章级预测,所以预测前不需要转化,代码要稍作修改
"""
import argparse
import json
...
def parse_arguments(parser):
    ...
    parser.add_argument('--pred_file', type=str,
                        default='result/chinese_result/seq.pred.json')
...
def toSegment(opinions):
    ...
    for i, opinion in enumerate(opinions):
        if i != 0:
            if opinion["event_id"] == result["event_id"] and opinion["doc_id"] == result["doc_id"] and opinion["start_sent_idx"] == result["end_sent_idx"]+1:
...
def compare(pred_opinions, gold_opinions):
    if('argument' in pred_opinions[0]):  # 任务2
        print("Task2")
        # Compute Task_F
        # 任务2的F1分数
        correct_num = 0
...
def evaluate(pred_file, gold_file):
    ...
    # pred_opinions = toSegment(raw_pred_opinions) #TODO 此句要视情况修改
    ...
    with open("pred_seg.json", 'w', encoding='utf-8') as f:
        f.write(json.dumps(pred_opinions, ensure_ascii=False, indent=2))
